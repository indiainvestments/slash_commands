...
Deno.test({
  ...
  fn: () => {
    ...
    const colors = Array(randomLength).fill("").map(() =>
      randomHexColor.next().value
    );
    const uniqueColors = [...new Set(colors)];
    assert(
      uniqueColors.length === randomLength,
      `repeated color generated in ${randomLength} color genration`,
    );
  },
});
...
Deno.test({
  ...
  fn: () => {
    ...
    assert(
      ...
      `path (${path}) weight (${weight}) does not match expected output (${expectedWeight})`,
    );
    path = "/contributors";
...
Deno.test({
  ...
  fn: () => {
    ...
    const testUrl = util.getGitbookSpaceUrl(
      urlObj1.apiUrl,
      urlObj1.version,
      urlObj1.spaceId,
      urlObj1.path,
      new URLSearchParams(urlObj1.params),
    );
    const expectedUrl =
      "https://api.xyz.com/v3/spaces/-zxmiorw-/page1/page2?query=abc";
    assert(
      testUrl.toString() === expectedUrl,
...
Deno.test({
  ...
  fn: () => {
    ...
    const testUrl = util.getGitbookSpaceUrl(
      ...
      urlObj.spaceId,
      urlObj.path,
      new URLSearchParams(urlObj.params),
    );
    const expectedUrl =
      "https://api.xyzabc.com/v5/spaces/-zxmiorw-/pageX/pageY?query=abc&id=testid";
    assert(
      testUrl.toString() === expectedUrl,
      "generated url does not match expected url",
    );
