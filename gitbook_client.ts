import { GitbookClientOptions, GitbookContent, GitbookPage, GitbookSearchNode, GitbookSpace, PathWeight } from './types/types.ts'
...
const getWeightOfPath = (path: string): number => {
    ...
    const pathValue = WEIGHTS.find((val, _) => {
        ...
        return searchPath.match(new RegExp(`^${valPath.startsWith('/') ? valPath.slice(1) : valPath}$`));
    });
